...
begin
  if mb.Active then begin
    ...
    StatusBar1.Panels[0].Text:='СОСТОЯНИЕ:  НЕАКТИВНО';
  end else begin
    mb.Address:= '10.10.120.118';
    mb.Port:=502;
    mb.DeviceAddr:=1;
    mb.Active:=true;
    StatusBar1.Panels[0].Text:='СОСТОЯНИЕ:  ';
    Button1.Caption:='Разъединить';
    {
    ListBox2.Clear;
    if mb.Active then begin
...
if mb.Vars[2].Value=1000 then
  Edit5.Text := 'закрыта' else Edit5.Text := 'открыта';

if mb.Vars[5].Value then
  Edit6.Text := 'закрыта' else Edit6.Text := 'открыта';
...
begin
 { if Pos('ОШИБКА!!!',ListBox2.Items[AVar.VarIndex])=0 then
    ListBox2.Items[AVar.VarIndex]:=ListBox2.Items[AVar.VarIndex]+'     ОШИБКА!!!';
 }
end;

procedure TForm1.Conn(Sender: TObject);
begin
  StatusBar1.Panels[0].Text:='СОСТОЯНИЕ:  СОЕДИНЕНО';
end;


procedure TForm1.Err(Sender: TObject; AErr: TErrorEvent);
begin
  StatusBar1.Panels[0].Text:='СОСТОЯНИЕ:  ОШИБКА СОЕДИНЕНИЯ!';
end;
...
{        try
  ...
  if  dOPCServer1.OPCGroups[1].OPCItems[0].Value=2 then begin
    Label4.Caption := 'Есть контакт';
    Label4.Font.Color := clGreen;  end else begin
    //Label4.Caption := 'Связи нет';
